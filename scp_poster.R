...
clean_data_ami_all_sup_shp_generation_utility <- clean_data_ami_all_sup_shp %>% filter(state_abbr=="CA", 
                                                                        county_name %in% 
                                                                          c("Sonoma County",
                                                                            "Mendocino County"),
...
clean_data_ami_all_sup_shp_nerc_region <- clean_data_ami_all_sup_shp %>% 
  filter(
    `NERC Region`=="WECC"
  )
...
code <- qr_code("https://sonomacleanpower.org/program-equity-framework")
# print(code)
# plot(code)
generate_svg(code, 
             filename = "qr.svg", 
...
# SCP subset map
poster_charts_generation_utility <- make_all_charts(
  clean_data=clean_data_ami_all_sup_shp_generation_utility, #clean_data_ami_all_sup_shp, 
...
ggsave("generation_utility_poster_map.svg", 
       plot=(poster_charts_generation_utility[["choropleth"]]  + theme(text = element_text(size = 20))),
       device = NULL,
       # path = NULL,
       scale = 1,
       width = 9,
...
# SERC Subset Map
poster_charts_nerc_region <- make_all_charts(
  clean_data=clean_data_ami_all_sup_shp_nerc_region, #clean_data_ami_all_sup_shp, 
  group_columns=c(NULL),
  metric_name=metric_name, 
  metric_long_name=metric_long_name,
  metric_label=metric_label,
...
ggsave("nerc_region_poster_map.svg", 
       plot=(poster_charts_nerc_region[["choropleth"]] + 
               theme(
