...
Of the above metrics, the Net Energy Return (NER) and Discretionary Energy Availability Rate (DEAR) are the most desirable for our assessment. The DEAR is nicely bound between 0-100% as a ratio that relates the available income remaining after bills are paid. The NER is the standard metric of Net Energy Analysis because it reflects the role of energy as an input to a value-generating process in ecosystems. EROI is a precursor of NER which has been replaced due to the increased focus on "net" rather than "gross" energy returns. Energy Burden is the metric of choice in the energy insecurity literature. It is a highly interpretable metric which helps policymakers and researchers understand the distribution and impacts of energy poverty throughout communities.
...
\[
NER^{*} = \frac{G_{income}-S_{energy}}{S_{energy}} 
s.t. \frac{S_{energy}}{G_{income}} > 10\%
\]
\[
